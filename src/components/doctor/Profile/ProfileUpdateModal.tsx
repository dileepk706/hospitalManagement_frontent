...
type ProfileUpdateModalprops = {
    doctor?: DoctorType;
    closeModal: () => void;
    // setUser: React.Dispatch<React.SetStateAction<DoctorType | undefined>>
}
...
const ProfileUpdateModal:React.FC<ProfileUpdateModalprops>=({doctor,closeModal})=> {
    ...
    const handleSubmit = async (user: initialValuesType) => {
        try {
            // setApiError('')
            // setApiSuccess('')
            // const updatedUser: DoctorType = await updateUser(user.name, user.email, user.age, user.phone, user.sex)
            // setUser(updatedUser)
